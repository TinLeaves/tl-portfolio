...
import { useScrollStaggerAnimation } from '../hooks/useScrollAnimation';
...
export default function SkillsGrid() {
  ...
  const { elementRef: sectionRef, isVisible, getItemProgress } = useScrollStaggerAnimation(allSkills.length, 0.1, 0.05);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 lg:gap-8">
            {allSkills.map((skill, skillIndex) => (
              <div 
                ...
                style={{
                  opacity: getItemProgress(skillIndex),
                  transform: `translateY(${(1 - getItemProgress(skillIndex)) * 20}px)`
                }}
              >
